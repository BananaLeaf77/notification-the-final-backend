...
type Subject struct {
	...
	Classes   []*Class       `gorm:"many2many:class_subjects" json:"classes"`
...
type Class struct {
	ClassID   int            `gorm:"primaryKey;autoIncrement" json:"class_id"`
	Name      string         `gorm:"type:varchar(3);not null;unique" json:"name" valid:"required~Class name is required"`
	Grade     int            `gorm:"not null" json:"grade" valid:"required~Grade is required"`
	Subjects  []*Subject     `gorm:"many2many:class_subjects" json:"subjects"`
...
type Student struct {
	...
	ClassID   int            `json:"class_id"`  // This will be set based on the class name lookup
	ClassName string         `json:"class"`      // Use this to accept the class name from the payload
...
type TestScore struct {
	...
	Teacher     User           `gorm:"references:UserID;constraint:OnUpdate:CASCADE,OnDelete:SET NULL" json:"teacher"` // The teacher who inputs the score
